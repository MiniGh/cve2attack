...
OUTPUT_DIR = Path(__file__).resolve().parent / "cve"
...
def extract_cpe_criteria(configurations):
    """从 configurations.nodes[].cpeMatch[].criteria 提取 CPE 列表。"""
    cpes = []
...
def main():
    ...
    for page in tqdm(range(total_pages), desc="Pages", unit="page"):
        ...
        for item in tqdm(data.get("vulnerabilities", []), desc=f"Page {page}", leave=False, unit="CVE"):
            ...
            # 提取 CPE criteria
            cpes = extract_cpe_criteria(cve_obj.get("configurations", []))
