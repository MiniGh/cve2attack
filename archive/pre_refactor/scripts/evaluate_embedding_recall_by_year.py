...
DEFAULT_PRED_DIR = PROJECT_ROOT / "output" / "retrieval"
DATA_REF_DIR = PROJECT_ROOT / "data_result"
CVE2ATTACK_REF_DIR = PROJECT_ROOT / "cve2attack_result"
DEFAULT_OUTPUT_MD = PROJECT_ROOT / "output" / "retrieval" / "2embedding_recall_by_year.md"
DEFAULT_OUTPUT_JSON = PROJECT_ROOT / "output" / "retrieval" / "2embedding_recall_by_year.json"
...
def write_report(rows: List[dict], skipped_years: List[str], output_md: Path) -> None:
    lines: List[str] = [
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pred-dir", type=Path, default=DEFAULT_PRED_DIR, help="Directory with prediction JSONL files.")
    parser.add_argument("--output-md", type=Path, default=DEFAULT_OUTPUT_MD, help="Output markdown report path.")
    parser.add_argument("--output-json", type=Path, default=DEFAULT_OUTPUT_JSON, help="Output JSON summary path.")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    pred_by_year = load_jsonl_directory(args.pred_dir)
    ...
    write_report(rows, skipped_years, args.output_md)
