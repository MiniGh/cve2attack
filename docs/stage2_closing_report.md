...
（CVE-2022-26904 的 T1548 对 T1068），当前 tactic 级 guard 结构上无法改善名次。

要在这类情形取得提升，需要能够区分机制的额外证据，例如漏洞组件与 Technique 机制之间的对应
关系，而不是继续调整 v2 权重。任何此类改动必须先形成独立假设与预注册，并在另一批案例上验证；
不得在本轮四例上调参后再把相同案例的结果宣称为独立效果。

其余待验证问题：更深、多分支、多 producer path 的攻击图是否会改变"深度 1 足够"的结论；如何取得
足够大的公开 Stage 2 样本集。
